...
public class TrueFalseFragment extends Fragment{
    ...
    public void setUpTrueFalseDrag(final ProblemTrueFalse problemTrueFalse,final ProblemListener listener){
            final Handler handler = new Handler();
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    if(view==null) {
                        setUpTrueFalseDrag(problemTrueFalse,listener);
                    }else{
                        Log.d("truefalsefragmentdrag","just a message");
